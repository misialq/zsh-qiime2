...
function set_up_q2_base_env() {
  ...
  if [[ "${ENV_NAME}" == "" ]]; then
    ENV_NAME="q2-${QIIME2_LATEST_DEV_RELEASE}"
  fi
...
function set_up_q2_full_env() {
  ...
  conda env create -n "qiime2-$VER_SHORT" --file "$ENV_FILE"
  rm "$ENV_FILE"
}
...
function build_conda_pkg() {
  ...
  if ! test -f "conda_build_config.yaml" || ! test -f "meta.yaml"
    then
